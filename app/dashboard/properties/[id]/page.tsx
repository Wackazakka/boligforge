...
function buildReviewSteps(imageIdx: number, avatarIdx: number): TourStep[] {
  ...
  if (avatarIdx >= 0) segmentSteg.push({ idx: avatarIdx, kjoer: () =>
    add('[data-tour="avatar-clip"]', `Se avataren i segment ${avatarIdx + 1}`,
      'Trykk «Forhåndsvis animasjonen» — det tar ett til tre minutter. Spill av klippet når det kommer; hjelpen blir stående. Er du ikke fornøyd, gir «Lag ny animasjon» et nytt forsøk, og det koster ingen video. Når du er ferdig her: trykk «Neste».') })
            ...
            <div data-tour="setting-pick" style={{ marginTop: '16px', paddingTop: '16px', borderTop: '1px solid var(--line)' }}>
              ...
              {VIS_KOMPOSITT_BAKGRUNN && showBgHint && (
                <div style={{
                  ...
                  borderRadius: '8px', fontSize: '12px', color: 'var(--ink)', lineHeight: '1.5',
                }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
                          {seg.clipUrl ? (
                            ...
                            <div style={{ position: 'relative', flexShrink: 0, borderRadius: '8px', overflow: 'hidden' }}>
                              {/* eslint-disable-next-line @next/next/no-img-element */}
                              <img
                                src={selectedAvatarUrl || effectivePortrait} alt="Avatar"
                                style={{ width: '96px', height: '64px', objectFit: 'cover', objectPosition: 'center 20%', display: 'block' }}
                              />
                              <button
                                onClick={e => { e.stopPropagation(); setLightboxUrl(selectedAvatarUrl || effectivePortrait || null) }}
                                title="Forstørr"
