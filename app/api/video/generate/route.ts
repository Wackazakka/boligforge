...
export async function POST(request: Request) {
  try {
    ...
    const { propertyId, script, voiceId, avatarImageUrl, portraitUrl, backgroundImageUrl, propertyImages, segments, outro, ambienceType, motion, motionStrength, segmentTransition, noMotionImages, recipe } = await request.json()
    ...
    const workerRes = await fetch(`${WORKER_URL}/jobs/video`, {
      ...
      body: JSON.stringify(useSegments
        ? { jobId, propertyId, avatarImageUrl, portraitUrl, backgroundImageUrl, voiceId, segments, outro, ...(ambienceType ? { ambienceType } : {}), ...(motion ? { motion: true, motionStrength: motionStrength || 'subtle' } : {}), ...(segmentTransition === 'fade' ? { segmentTransition } : {}), ...(Array.isArray(noMotionImages) && noMotionImages.length ? { noMotionImages } : {}) }
