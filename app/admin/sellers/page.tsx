...
const input: React.CSSProperties = { background: '#0f0f10', border: '1px solid #3f3f46', borderRadius: 8, padding: '10px 12px', color: '#f0f0f0', fontSize: 14, outline: 'none', width: '100%' }
const label: React.CSSProperties = { fontSize: 12, fontWeight: 700, color: '#71717a', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 6, display: 'block' }
const btn: React.CSSProperties = { background: '#2563eb', color: '#fff', border: 'none', borderRadius: 8, padding: '11px 20px', fontWeight: 700, fontSize: 14, cursor: 'pointer' }
  ...
  return (
    <div>
      ...
      <div style={card}>
        ...
        <form onSubmit={createSeller} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 12, marginTop: 6 }}>
              {([['År 1', rateY1, setRateY1], ['År 2', rateY2, setRateY2], ['År 3', rateY3, setRateY3], ['År 4 og senere', rateY4, setRateY4]] as const).map(([lbl, val, set]) => (
                <div key={lbl}>
                  <div style={{ fontSize: 11, color: '#71717a', marginBottom: 4 }}>{lbl}</div>
                  <div style={{ position: 'relative' }}>
                    ...
                    <span style={{ position: 'absolute', right: 12, top: 11, color: '#71717a', fontSize: 13 }}>%</span>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
            ...
            <span style={{ fontSize: 12, color: '#71717a' }}>Ref-kode: 3–40 tegn (a–z, 0–9, - _). Trappen teller fra hver kundes (org) registrering.</span>
          </div>
        </form>
        {createMsg && <div style={{ marginTop: 12, fontSize: 13, color: createMsg.ok ? '#4ade80' : '#f87171', wordBreak: 'break-all' }}>{createMsg.text}</div>}
      </div>
      ...
      <div style={{ ...card, display: 'flex', alignItems: 'end', gap: 12, flexWrap: 'wrap' }}>
        ...
        <button style={{ ...btn, background: '#3f3f46', opacity: computing ? 0.7 : 1 }} disabled={computing} onClick={computeCommissions}>{computing ? 'Beregner...' : 'Beregn'}</button>
        {computeMsg && <div style={{ fontSize: 13, color: '#a1a1aa', flex: 1 }}>{computeMsg}</div>}
      </div>

      <div style={card}>
        <h2 style={{ margin: '0 0 16px', fontSize: 16, fontWeight: 800 }}>Alle selgere ({sellers.length})</h2>
        {loading ? <p style={{ color: '#71717a' }}>Laster...</p> : sellers.length === 0 ? (
          <p style={{ color: '#71717a', fontSize: 14 }}>Ingen selgere ennå.</p>
        ) : (
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
              <thead><tr style={{ textAlign: 'left', color: '#71717a', fontSize: 12, textTransform: 'uppercase', letterSpacing: 1 }}>
              ...
              <tbody>
                {sellers.map(s => (
                  <tr key={s.id} style={{ borderTop: '1px solid #27272a' }}>
                    ...
                    <td style={{ padding: 8, fontFamily: 'monospace', color: '#60a5fa' }}>{s.ref_code}</td>
                    <td style={{ padding: 8, fontFamily: 'monospace', fontSize: 12, color: '#a1a1aa' }}>{schedule(s)}</td>
                    <td style={{ padding: 8, fontSize: 12, color: '#a1a1aa' }}>{s.manager_rate != null ? `Sjef ${pct(s.manager_rate)}%` : (s.parent_name ?? '—')}</td>
                    <td style={{ padding: 8 }}>{s.customer_count}</td>
                    <td style={{ padding: 8 }}>{Number(s.total_commission).toLocaleString('nb-NO')} kr</td>
                    <td style={{ padding: 8 }}><span style={{ fontSize: 12, padding: '3px 8px', borderRadius: 999, background: s.active ? '#064e3b' : '#3f1d1d', color: s.active ? '#6ee7b7' : '#fca5a5' }}>{s.active ? 'aktiv' : 'inaktiv'}</span></td>
                    <td style={{ padding: 8 }}><a href={`/admin/sellers/${s.id}`} style={{ color: '#2563eb', textDecoration: 'none', fontWeight: 600 }}>Detaljer →</a></td>
                  </tr>
                ))}
              </tbody>
            </table>
