 ...
 * («should only be used for development purposes»). Når Facebook, Instagram
 * eller LinkedIn henter videoen derfra, kan hentingen ta minutter — og feile
 * helt når dagens kvote er brukt opp. Det stoppet tre publiseringer på rad i
 * ContentForge 22.08.2026; ReelHome bruker samme domene i produksjon.
 *
 * Denne bytter dev-URL-en mot en presignert URL rett mot R2-endepunktet
 * (målt 0,6 s mot 2+ min). Ingen lagrede URL-er endres — omskrivingen skjer
 * bare i det øyeblikket en URL gis videre til en ekstern plattform.
 *
...
export async function hentbarMediaUrl(url: string): Promise<string> {
  try {
    ...
    console.log('[r2-presign] Omskrev dev-URL til presignert R2-URL:', key)
    return signert
  } catch (err) {
    console.error('[r2-presign] Presign feilet, bruker original URL:', err)
    return url
  }
}
